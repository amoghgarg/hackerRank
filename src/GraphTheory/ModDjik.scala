...
class BinaryHeap[T]{
  ...
  def delete(key: T){
    try{
      ...
      case e: NoSuchElementException => {println("No Such Elem")}
    }
  }
}
 ...
 * Same as the Djikatra, but a modified cost system.
 * The cost of an edge is equal to the Cost uptill That Point - Weight of the Edge.
 * If the diffrence is negative, the cost is taken as 0
 * So the cost of an edge = max(0, CostUptilThatPoint - WeightOfTheEdge)
 */
object ModDjik {
  ...
  def main(args: Array[String]) = {
    ...
    (1 to edgeCount).foreach(i => {
      ...
      adjList.update(a, adjList(a) + (b -> cost) )
      adjList.update(b, adjList(b) + (a -> cost) )

    })

    //Start applying the Djiktras Algo
    val queue = new BinaryHeap[Int]()   //Array of (node, costUptilNow)
      ...
      if(currentNode == 0){
        ...
        adjNodes.foreach(edge => {
          ...
          if(costs(edge._1) == -1){
            ...
            //          val currentCost = queue.get(edge._1)    // theCurrentCost of this node
            val edgeWeight = adjList(currentNode).get(edge._1)
            val edgeCost =  - costTillNow + edgeWeight.get
            val proposedCost = if(edgeCost > 0){
              costTillNow + edgeCost
            } else{
              costTillNow
            }

            queue.push(edge._1, proposedCost)
          }
...
//    costs.foreach(println)
    val finalans = costs(nodeCount-1)
